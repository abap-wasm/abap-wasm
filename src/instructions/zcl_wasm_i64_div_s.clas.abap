...
CLASS zcl_wasm_i64_div_s IMPLEMENTATION.
  ...
  METHOD zif_wasm_instruction~execute.
    ...
    DATA(lv_val1) = CAST zcl_wasm_i64( io_memory->get_stack( )->stack_pop( ) )->get_signed( ).
    DATA(lv_val2) = CAST zcl_wasm_i64( io_memory->get_stack( )->stack_pop( ) )->get_signed( ).
...
* division is truncating, so round towards zero
    IF sign( lv_val1 ) <> sign( lv_val2 ).
      io_memory->get_stack( )->stack_push( zcl_wasm_i64=>from_signed( -1 * ( abs( lv_val2 ) DIV abs( lv_val1 ) ) ) ).
    ELSE.
      io_memory->get_stack( )->stack_push( zcl_wasm_i64=>from_signed( lv_val2 DIV lv_val1 ) ).
    ENDIF.
